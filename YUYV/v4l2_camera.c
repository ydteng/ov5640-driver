...
static int v4l2_set_format(void)
{
    ...
    if (V4L2_CAP_TIMEPERFRAME & streamparm.parm.capture.capability) {
        ...
        /* 新增 设置默认格式为0*/
        streamparm.parm.capture.capturemode = 10;
...
{
    struct v4l2_buffer buf = {0};
    unsigned char *v4l2_ptr;
    unsigned short *lcd_ptr;
    int i, j;

    // 假设：
    // width, height 是 LCD 的分辨率 (如 800x480)
    // frm_width, frm_height 是摄像头输出的分辨率 (如 640x480)
    ...
    for ( ; ; ) {
        for(buf.index = 0; buf.index < FRAMEBUFFER_COUNT; buf.index++) {
            if (ioctl(v4l2_fd, VIDIOC_DQBUF, &buf) < 0) break;

            // 1. 获取当前帧 V4L2 缓冲区的起始地址
            unsigned char *frame_start = (unsigned char *)buf_infos[buf.index].start;
            
            for (j = 0; j < min_h; j++) {
                // 2. 关键点：每一行都根据索引 j 重新计算起始地址，防止累加偏移
                // lcd_ptr 指向 LCD 的第 j 行开头
                lcd_ptr = screen_base + (j * width); 
                
                // v4l2_ptr 指向摄像头数据的第 j 行开头
                // YUYV 每个像素占 2 字节，所以一行是 frm_width * 2 字节
                v4l2_ptr = frame_start + (j * frm_width * 2);

                for (i = 0; i < min_w; i += 2) {
                    // 每次处理 4 字节 YUYV -> 2 个像素 (RGB565)
                    int y0 = v4l2_ptr[0];
                    int u  = v4l2_ptr[1] - 128;
                    int y1 = v4l2_ptr[2];
                    int v  = v4l2_ptr[3] - 128;
                    v4l2_ptr += 4;

                    // 像素 1 转换
                    int r = CLAMP(y0 + ((1402 * v) >> 10));
                    int g = CLAMP(y0 - ((344 * u + 714 * v) >> 10));
                    int b = CLAMP(y0 + ((1772 * u) >> 10));
                    lcd_ptr[i] = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3);

                    // 像素 2 转换
                    r = CLAMP(y1 + ((1402 * v) >> 10));
                    g = CLAMP(y1 - ((344 * u + 714 * v) >> 10));
                    b = CLAMP(y1 + ((1772 * u) >> 10));
                    lcd_ptr[i + 1] = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3);
                }
            }
